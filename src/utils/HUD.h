...
public:
    ...
    void draw(int screenW, int screenH, const char* line1, const char* line2, const char* line3)
    {
        ...
        auto measure = [&](const char* s)->int 
        {
            ...
            { 
                total += glyph(*p).advance * scale; ++count; 
            }
        ...
        int lines = 0; if (line1 && *line1) ++lines; if (line2 && *line2) ++lines; if (line3 && *line3) ++lines;
        ...
        glDrawArraysInstanced(GL_TRIANGLES, 0, 6, (GLsizei)rects.size());
...
private:
    ...
    struct Glyph { uint8_t row[7]; uint8_t advance; };
    ...
    static inline const Glyph& glyph(char c)
    {
        ...
        if (!init)
        {
            auto set = [&](char ch, std::initializer_list<uint8_t> rows, uint8_t adv = 6)
                { Glyph g{}; int k = 0; for (uint8_t r : rows) g.row[k++] = r; g.advance = adv; G[(int)ch] = g; };
            ...
            set('m', { 0b00000,0b00000,0b11010,0b10101,0b10101,0b10101,0b10101 }, 6);
            set('s', { 0b00000,0b00000,0b01110,0b10000,0b01110,0b00001,0b11110 });
